...
class DataLoaderS(object):
    ...
    def __init__(self, file_name, train, valid, device, horizon, window, normalize=2, out=1, col_file=None):
        ...
        # Keep these statistics on a single device (CPU) to avoid cuda/cpu mismatch.
        scale_cpu = self.scale.detach().to("cpu") if isinstance(self.scale, torch.Tensor) else self.scale
        if self.test[1].numel() > 0:
            y_cpu = self.test[1].detach().to("cpu")
            tmp = y_cpu * scale_cpu.expand(y_cpu.size(0), y_cpu.size(1), self.m)
        else:
            # When seq_out_len >= test period, batched test is empty; use test_window instead
            tw_cpu = self.test_window.detach().to("cpu")
            tmp = tw_cpu * scale_cpu.expand(tw_cpu.size(0), self.m)
