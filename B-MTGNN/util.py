...
class DataLoaderS(object):
    ...
    def _normalized(self, normalize):
        ...
        if (normalize == 0):
            ...
            self.mean = np.zeros(self.m)  # 수정: 평균값 초기화
        ...
        # 수정: Phase 2 - StandardScaler 정규화 (평균=0, 표준편차=1)
        if (normalize == 3):
            self.mean = np.zeros(self.m)  # 평균값 저장용
            for i in range(self.m):
                mean = np.mean(self.rawdat[:, i])
                std = np.std(self.rawdat[:, i])
                self.mean[i] = mean  # 평균 저장
                
                if std > 1e-6:  # 표준편차가 0이 아닌 경우 (일반 환율)
                    self.dat[:, i] = (self.rawdat[:, i] - mean) / std
                    self.scale[i] = std  # 역정규화를 위해 std 저장
                else:  # 표준편차가 0인 경우 (us_fx=1 같은 상수)
                    self.dat[:, i] = 0  # 0으로 변환 (노이즈 제거)
                    self.scale[i] = 1
